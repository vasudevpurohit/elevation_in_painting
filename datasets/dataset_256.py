...
class ImageFolderMaskDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # adding path to custom masks and loading them
        if self._path == 'data/new_360_data/training_data/maps':
            file_name='/home/vpurohi/Desktop/cpsc8810_proj/MAT/MAT/data/new_360_data/training_data/masks/mask_{}.jpg'.format(idx)
            mask=np.asarray(PIL.Image.open(file_name),dtype=np.float32)
            mask=mask[:,:,0]
            mask=np.reshape(mask,(1,256,256))
            mask[mask>0]=1
        elif self._path == 'data/new_360_data/validation_data/maps':
            file_name='/home/vpurohi/Desktop/cpsc8810_proj/MAT/MAT/data/new_360_data/validation_data/masks/mask_{}.jpg'.format(idx)
            mask=np.asarray(PIL.Image.open(file_name),dtype=np.float32)
            mask=mask[:,:,0]
            mask=np.reshape(mask,(1,256,256))
            mask[mask>0]=1 
...
if __name__ == '__main__':
    res = 256
    dpath = '/data/new_360_data/validation_data/maps'
